...
class BagleyApp(App):
    ...
    BINDINGS = [
        ...
        # Mode switches (priority so they bypass focused Input)
        Binding("alt+1", "set_mode(1)", "", show=False, priority=True),
        Binding("alt+2", "set_mode(2)", "", show=False, priority=True),
        Binding("alt+3", "set_mode(3)", "", show=False, priority=True),
        Binding("alt+4", "set_mode(4)", "", show=False, priority=True),
        Binding("alt+5", "set_mode(5)", "", show=False, priority=True),
        Binding("alt+6", "set_mode(6)", "", show=False, priority=True),
        Binding("alt+7", "set_mode(7)", "", show=False, priority=True),
        Binding("alt+8", "set_mode(8)", "", show=False, priority=True),
        Binding("alt+9", "set_mode(9)", "", show=False, priority=True),
        Binding("ctrl+m", "cycle_mode", "Cycle mode", show=False, priority=True),
        # Tab management
    ...
    async def on_key(self, event) -> None:
        """Plain digits 1-9 switch mode when focus is not on an Input."""
        if event.key in "123456789":
            focused = self.focused
            if focused is None or not isinstance(focused, Input):
                self.action_set_mode(int(event.key))
                event.stop()
